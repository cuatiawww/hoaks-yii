...
{
    public $master_wilayah_id;
    public $tbl_wilayah_id;
    public $kd_prop;
    public $kd_kab;
    public $kd_kecamatan;
    ...
    public function rules()
    {
        return [
            ...
            [['level_user_id', 'id_user_level'], 'integer'],
            [['kd_prop', 'kd_kab', 'kd_kecamatan', 'master_wilayah_id', 'tbl_wilayah_id'], 'safe'],
            [['is_active'], 'boolean'],
        ];
    }
    ...
    public function getMasterWilayahNama()
    {
        if ($this->kd_kab) {
            $row = (new \yii\db\Query())->select('name')->from('wilayah_kabupaten')->where(['code' => $this->kd_kab])->one();
            return $row ? $row['name'] : '';
        }
        if ($this->kd_prop) {
            $row = (new \yii\db\Query())->select('name')->from('wilayah_provinsi')->where(['code' => $this->kd_prop])->one();
            return $row ? $row['name'] : '';
        }
        return '';
    }
