...
class UserModelController extends BaseController
{
    ...
    public function actionSave()
    {
        ...
        try {
            ...
            if (!$this->isWilayahOptionalForLevel($levelUserId)) {
                if ($this->isProvinsiLevel($levelUserId) && !$kdProp) {
                    return ['success' => false, 'message' => 'Provinsi wajib dipilih'];
                }
                if ($this->isKabKotaLevel($levelUserId) && !$kdKab) {
                    return ['success' => false, 'message' => 'Kabupaten/Kota wajib dipilih'];
                }
                if (!$this->isValidWilayahSelection($kdProp, $kdKab, $levelUserId)) {
                    return ['success' => false, 'message' => 'Wilayah yang dipilih tidak valid'];
                }
            }
            ...
            if ($model->save()) {
                ...
                if ($isCreate && !empty(trim((string) $model->getAttribute('email')))) {
                    $emailSent = EmailService::sendAdminCreatedAccount($model, $password);
    ...
    public function actionCreate()
    {
        ...
        if ($this->request->isPost) {
            ...
            if (!$this->isWilayahOptionalForLevel($levelUserId)) {
                if ($this->isProvinsiLevel($levelUserId) && !$model->getAttribute('kd_prop')) {
                    $model->addError('master_wilayah_id', 'Provinsi wajib dipilih');
                } elseif ($this->isKabKotaLevel($levelUserId) && !$model->getAttribute('kd_kab')) {
                    $model->addError('master_wilayah_id', 'Kabupaten/Kota wajib dipilih');
                } elseif (!$this->isValidWilayahSelection($model->getAttribute('kd_prop'), $model->getAttribute('kd_kab'), $levelUserId)) {
                    $model->addError('master_wilayah_id', 'Wilayah yang dipilih tidak valid');
                }
            }
    ...
    protected function isValidWilayahSelection($kdProp, $kdKab, $level): bool
    {
        if ($this->isWilayahOptionalForLevel($level)) return true;

        if ($this->isProvinsiLevel($level)) {
            return !empty($kdProp) && (new \yii\db\Query())->from('wilayah_provinsi')->where(['code' => $kdProp])->exists();
        }
        if ($this->isKabKotaLevel($level)) {
            return !empty($kdKab) && (new \yii\db\Query())->from('wilayah_kabupaten')->where(['code' => $kdKab, 'parent_code' => $kdProp])->exists();
        }
        return true;
    }
    ...
    public function getProvinsiOptions(): array
    {
        ...
        try {
            $rows = (new \yii\db\Query())
                ...
                ->all();
            foreach ($rows as $row) $list[$row['code']] = $row['name'];
        } catch (\Throwable $e) {
